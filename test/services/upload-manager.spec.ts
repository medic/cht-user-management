...
describe('services/upload-manager.ts', () => {
  ...
  for (const scenario of coreScenarios) {
    describe(`cht-core version ${scenario}`, () => {
      it('mock data is properly sent to chtApi', async () => {
        const { fakeFormData, contactType, chtApi, sessionCache, remotePlace } = await createMocks(scenario);
      ...
      it('mock data is properly sent to chtApi (sessionCache cache)', async () => {
        const { fakeFormData, contactType, sessionCache, chtApi, remotePlace } = await createMocks(scenario);
      ...
      it('uploads in batches', async () => {
        const placeCount = 11;
        const { fakeFormData, contactType, sessionCache, chtApi } = await createMocks(scenario);
        const place = await PlaceFactory.createOne(fakeFormData, contactType, sessionCache, chtApi);
      ...
      it('required attributes can be inherited during replacement', async () => {
        const { remotePlace, sessionCache, contactType, fakeFormData, chtApi } = await createMocks(scenario);
      ...
      it('place with validation error is not uploaded', async () => {
        const { sessionCache, contactType, fakeFormData, chtApi } = await createMocks(scenario);
      ...
      it('uploading a chu and dependant chp where chp is created first', async () => {
        const { remotePlace, sessionCache, chtApi } = await createMocks(scenario);
...
async function createMocks(coreVersion: string) {
