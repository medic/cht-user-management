  ...
  for (const scenario of CORE_SCENARIOS) {
    describe(`cht-core version ${scenario}`, () => {
      it('mock data is properly sent to chtApi - standard', async () => {
        const { fakeFormData, contactType, chtApi, sessionCache, subcounty } = await createMocks(scenario);
      ...
      it('mock data is properly sent to chtApi - sessionCache cache', async () => {
        const { fakeFormData, contactType, sessionCache, chtApi, subcounty } = await createMocks(scenario);
      ...
      it('required attributes can be inherited during replacement', async () => {
        const { subcounty, sessionCache, contactType, fakeFormData, chtApi } = await createMocks(scenario);
      ...
      it('uploading a chu and dependant chp where chp is created first', async () => {
        const { subcounty, sessionCache, chtApi } = await createMocks(scenario);
