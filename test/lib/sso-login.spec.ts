  ...
  describe('ssoLogin', () => {
    it('nominal: follows redirects, returns sessionToken and username', async () => {
      ...
      stub.getCalls().forEach(call => {
        ...
        const auth = (call.args[0].headers as any)?.Authorization;
        if (onIdp) {
    ...
    it('accepts any origin in the allowlist', async () => {
      ...
      expect((idpCall!.args[0].headers as any)?.Authorization).to.eq(`Bearer ${ACCESS_TOKEN}`);
    });
    ...
    it('replays cookies on subsequent requests to the same host', async () => {
      ...
      const secondHopHeaders = stub.getCall(1).args[0].headers as any;
